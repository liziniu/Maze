...
class Acer:
    ...
    def call(self, replay_start, nb_train_epoch):
        ...
        obs, actions, rewards, mus, dones, masks, goal_obs = self.adjust_shape(results)
        names_ops, values_ops = model.train_policy(
            obs, actions, rewards, dones, mus, model.initial_state, masks, steps, goal_obs)
    ...
    def adjust_shape(self, results):
        ...
        obs = obs.reshape((runner.nenv*(runner.nsteps+1), ) + runner.obs_shape)
        ...
        goal_obs = results["goal_obs"].reshape((runner.nenv*(runner.nsteps+1),) + runner.obs_shape)
        return obs, actions, rewards, mus, dones, masks, goal_obs
